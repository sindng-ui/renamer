...
- **Keywords**: [`빠른 실행`, `원클릭 재실행`, `네온 서클 버튼`, `이력 기억`, `localStorage`, `자동 분기`, `실시간 전량 스캔`, `즉시 원버튼 생성`, `스마트 칩`, `No-Clipping Layout`]
- **Location**:
  - `View`: [src/components/QuickRunView.tsx](./src/components/QuickRunView.tsx)
- **Core Interface**:
  - `QuickRunView`: 이전 리네임 대상 폴더 경로 및 적용 규칙을 `localStorage`에서 불러와 원버튼으로 구성. 폴더를 처음 선택하는 순간(`handleFilesSelected`)에도 즉시 `lastJob`을 생성/갱신하여, 앱 재실행이나 변환 작업 없이도 0번 탭으로 이동 시 **원버튼(⚡ 즉시 전체 변환하기)**과 대상 폴더 정보가 즉시 렌더링되도록 보장. 변환 중에는 불필요한 카드를 가리고 중앙 서클+프로그레스 스트림만 노출하며, 변환 완료 시 `ResultSummary` 내부 스크롤과 하단 고정 버튼을 적용.
